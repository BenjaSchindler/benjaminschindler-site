  ...
  return (
    ...
    >
      <button
        ...
        onClick={() => setLang("en")}
        className={`inline-flex min-h-8 items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-semibold tracking-wider uppercase transition-colors ${
          lang === "en"
            ? "bg-[var(--surface-raised)] text-[var(--foreground)]"
            : "text-[var(--foreground-muted)] hover:text-[var(--foreground-dim)]"
        }`}
      >
        {!compact && <FlagUS className="h-2.5 w-[15px] rounded-[1px] shrink-0" />}
        <span>EN</span>
      </button>
      <button
        type="button"
      ...
      >
        {!compact && <FlagES className="h-2.5 w-[15px] rounded-[1px] shrink-0" />}
        <span>ES</span>
      </button>
    </div>
  );
